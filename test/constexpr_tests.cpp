...
TEST_CASE("test memory writes")
{
  ...
  REQUIRE(static_test<systest6.RAM[100] == 5>());
}
...
TEST_CASE("Test arbitrary code execution with loop")
{
  ...
  */
  ...
  REQUIRE(static_test<system.RAM[100] == 0>());
  REQUIRE(static_test<system.RAM[104] == 4>());
  REQUIRE(static_test<system.RAM[105] == 0>());
  REQUIRE(static_test<system.RAM[106] == 1>());
}


void test_condition_parsing()
{
  REQUIRE(static_test<Instruction{ 0b1110'1010'0000'0000'0000'0000'0000'1111 }.get_condition() == Condition::AL>());
}
